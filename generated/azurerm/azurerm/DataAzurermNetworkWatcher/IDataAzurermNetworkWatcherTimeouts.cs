using Amazon.JSII.Runtime.Deputy;

#pragma warning disable CS0672,CS0809,CS1591

namespace azurerm.DataAzurermNetworkWatcher
{
    [JsiiInterface(nativeType: typeof(IDataAzurermNetworkWatcherTimeouts), fullyQualifiedName: "azurerm.dataAzurermNetworkWatcher.DataAzurermNetworkWatcherTimeouts")]
    public interface IDataAzurermNetworkWatcherTimeouts
    {
        /// <summary>Docs at Terraform Registry: {@link https://registry.terraform.io/providers/hashicorp/azurerm/4.1.0/docs/data-sources/network_watcher#read DataAzurermNetworkWatcher#read}.</summary>
        [JsiiProperty(name: "read", typeJson: "{\"primitive\":\"string\"}", isOptional: true)]
        [Amazon.JSII.Runtime.Deputy.JsiiOptional]
        string? Read
        {
            get
            {
                return null;
            }
        }

        [JsiiTypeProxy(nativeType: typeof(IDataAzurermNetworkWatcherTimeouts), fullyQualifiedName: "azurerm.dataAzurermNetworkWatcher.DataAzurermNetworkWatcherTimeouts")]
        internal sealed class _Proxy : DeputyBase, azurerm.DataAzurermNetworkWatcher.IDataAzurermNetworkWatcherTimeouts
        {
            private _Proxy(ByRefValue reference): base(reference)
            {
            }

            /// <summary>Docs at Terraform Registry: {@link https://registry.terraform.io/providers/hashicorp/azurerm/4.1.0/docs/data-sources/network_watcher#read DataAzurermNetworkWatcher#read}.</summary>
            [JsiiOptional]
            [JsiiProperty(name: "read", typeJson: "{\"primitive\":\"string\"}", isOptional: true)]
            public string? Read
            {
                get => GetInstanceProperty<string?>();
            }
        }
    }
}
