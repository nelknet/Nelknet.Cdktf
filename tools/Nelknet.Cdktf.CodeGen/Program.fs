...
let generateProvider (options: GeneratorOptions) =
    ...
    let builders =
        ...
        |> List.choose(fun resource ->
            let hasResource = typeMap.ContainsKey(resource.ResourceFqn)
            let hasConfig = typeMap.ContainsKey(resource.ConfigFqn)

            if hasResource && hasConfig then
                let props = interfaces[resource.ConfigFqn]
                Some(buildBuilderDefinition options.ModuleName dotnetNamespace typeMap resource.BaseType resource.ResourceType resource.ConfigType resource.ResourceName resource.Docs props)
            else
                let missingParts =
                    [ if not hasResource then yield sprintf "%s (resource)" resource.ResourceFqn
                      if not hasConfig then yield sprintf "%s (config)" resource.ConfigFqn ]
                    |> String.concat ", "

                printfn "Skipping %s: missing bindings for %s" resource.ResourceFqn missingParts

                let placeholderPath =
                    Path.Combine(outputRoot, "Resources", resource.ResourceName + ".fs")

                Directory.CreateDirectory(Path.GetDirectoryName(placeholderPath)) |> ignore

                let placeholderContent =
                    [ "namespace Nelknet.Cdktf.Providers"
                      ""
                      sprintf "// Resource '%s' skipped: missing bindings for %s" resource.ResourceFqn missingParts
                      "// Generated by Nelknet.Cdktf.CodeGen."
                      ""
                      sprintf "module internal %sPlaceholder =" resource.ResourceName
                      "    ()" ]

                File.WriteAllLines(placeholderPath, placeholderContent)
                None)
